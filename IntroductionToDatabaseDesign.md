...
1.	**Do any Entities define the entity?** If the answer is **NO** then the entity concerned is in all likelihood an attribute or at the very least the beginning of an attribute structure.  If the answer is **YES** then the Entity concerned inherits its parent entitys primary key as part of its own.  It may still be part of an attribute structure or at the very least the final child table which is often a transactional table.
